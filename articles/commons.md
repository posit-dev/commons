...
A table’s dictionary entry can also carry `definitions`: named, governed
SQL expressions over that table’s rows, each with a declared type.
Boolean definitions act as filters, aggregates as metrics, and
everything else as derived dimensions. An expression can reference a
sibling definition by its bare name.
...
The model writes a definition as a `{{name}}` token anywhere in its
`run_sql` SQL — `WHERE {{realized}}`,
`SELECT {{net_revenue}} AS net_revenue` — and commons expands the token
to the governed expression before the query runs, so the trusted
fragment lands byte-exact and the tool result records which definitions
were applied. Each definition is validated against the live source (a
typo’d column fails agent construction with the definition named),
listed briefly in the system prompt, delivered in full with its table’s
first-touch entry, and searchable with the rest of the dictionary.

Metrics are also queryable directly: the `call_metrics` tool takes
metric, dimension, and filter names plus simple column predicates, and
commons compiles and runs the grouped query — a governed call in the
shape of mainstream semantic-layer queries, with every identifier drawn
from the dictionary. The `search_pool` tool searches measures and
definitions together, labeling each result with its kind and how to
invoke it.

## Defining the semantic layer

The semantic layer is the agent’s pool of trusted operations. Governed
definitions (above) cover computations expressible as SQL over
documented tables; measures cover everything R can do beyond that. A
measure is an ordinary R function wrapped with metadata that tells the
model what it computes and what arguments it accepts — reach for one
when a computation needs call-time parameters or methodology that
shouldn’t live in SQL (model fits, forecasts, multi-source stitching).
...
The `log` argument controls whether commons records conversation
trajectories. It is `FALSE` by default. With `log = TRUE`, trajectories
...
reading them requires editor-level access. To let named colleagues
review them, pass their Connect usernames to `share_with` and they’ll be
added as collaborators on the content:
...
Note that a colleague whose Connect *account* role is viewer cannot read
trajectories even when named in `share_with`; trajectory readers need at
least a publisher account. `share_with` only applies on Connect; off
Connect, commons warns and ignores it.

Read logged trajectories back with
[`read_trajectories()`](https://solid-adventure-ny1mpqy.pages.github.io/reference/read_trajectories.md).
With no arguments, it reads this content’s traces when running on
Connect, the deployed content’s traces when run in a project that’s been
deployed with rsconnect (set `CONNECT_API_KEY`), and local trace files
otherwise.
