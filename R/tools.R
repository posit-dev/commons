...
tool_run_sql <- function(private) {
  ellmer::tool(
    ...
    "Run a read-only DuckDB SELECT query against the data source. Use this when no registered measure answers the question.",
    arguments = list(
      sql = ellmer::type_string("A DuckDB-dialect SELECT query.")
    ),
    name = "run_sql",
    annotations = ellmer::tool_annotations(
      title = "SQL",
