...
databricks_table_registry <- function(
  ...
  if (is.null(tables)) {
    ids <- list(databricks_current_namespace(con, call = call))
  } else {
  ...
  validate <- list()
  for (id in ids) {
    type <- databricks_id_type(id, call = call)
    if (identical(type, "relation")) {
      relations[[length(relations) + 1L]] <- databricks_exact_relation(
        con,
        id,
        call = call
      )
    ...
    relations <- c(
      relations,
      databricks_list_relations(con, id, call = call)
    )
  }
