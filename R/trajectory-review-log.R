read_review_state <- function(review_dir, call = rlang::caller_env()) {
  if (!dir.exists(review_dir)) {
    return(list(flags = character(), notes = list()))
  }

  files <- review_document_files(review_dir)
  documents <- lapply(files, read_review_document, call = call)
...
review_document_body <- function(id, turns, flags, notes) {
  ...
  sections <- c(
    ...
    review_notes_markdown(conversation_notes)
  )
...
review_exchange_markdown <- function(
  ...
  c(
    ...
    "### User",
    "",
    review_blockquote(exchange[[1]]@text),
    contents,
    citation_section
  )
}
  ...
  for (turn in exchange) {
    for (content in turn@contents) {
      if (
        ...
        sections <- c(sections, review_blockquote(content@text))
        assistant_open <- TRUE
...
review_notes_markdown <- function(notes) {
  if (length(notes) == 0) {
    return(c("", "**Review notes:** None"))
  }

  rendered <- unlist(lapply(notes, function(note) {
    c(
      "",
      review_blockquote(note$note),
      ">",
      sprintf("> _%s, %s_", note$user, note$time)
    )
  }))
  c("", "**Review notes:**", rendered)
}

review_blockquote <- function(text) {
  lines <- strsplit(text, "\n", fixed = TRUE)[[1]]
...
read_review_document <- function(file, call = rlang::caller_env()) {
  lines <- tryCatch(
    readLines(file, warn = FALSE, encoding = "UTF-8"),
    error = function(error) {
      cli::cli_abort(
        "Could not read trajectory review document {.file {file}}.",
        parent = error,
        call = call
      )
    }
  ...
  if (length(lines) < 3 || !identical(lines[[1]], "---")) {
    cli::cli_abort(
      "Malformed trajectory review document {.file {file}}.",
      call = call
    )
  }
