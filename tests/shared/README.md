...
- **Warehouse lowering.** `definition-warehouse-sql.json` pins the Snowflake and Databricks SQL a governed definition compiles to, keyed to the same corpus dictionaries. It differs from every other fixture here in what backs it. The pinned data-dict binary emits `SQL(duckdb)` and R targets only, so there is no upstream output to generate from and no authority to check against: these emitters are commons-only lowering. The fixture is therefore hand-maintained and is the agreement between the two implementations rather than a record of data-dict's. See [commons #g0ax](https://github.com/posit-dev/commons/issues) in the tracker for the upstream ask that would let one Rust implementation serve both languages and delete both emitters. Cases pin `code` exactly where the whole string is worth reading, and `code_contains` or `notes_contain` where a rule is what matters and the full rendering would obscure it.
