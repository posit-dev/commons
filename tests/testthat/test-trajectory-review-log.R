...
test_that("review directories resolve from arguments and the environment", {
  ...
  expect_equal(
    resolve_review_dir("/explicit/reviews"),
    "/explicit/reviews"
  )
})

test_that("review state ignores unrelated Markdown and rejects malformed reviews", {
  review_dir <- withr::local_tempdir()
  ...
  expect_error(
    read_review_state(review_dir),
    "Malformed trajectory review document"
  )
  unlink(invalid)
  expect_equal(
    read_review_state(review_dir),
    list(flags = character(), notes = list())
  )
})
