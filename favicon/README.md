...
pkgdown template writes them for the R site and `docs/index.html` does it for
the landing page. The Python site's are generated by great-docs, which derives
its own icon set from the `favicon:` source named in `pkg-py/great-docs.yml`;
point that at a PNG rather than the SVG, because great-docs rasterizes an SVG
with cairosvg, which needs a system cairo library.
