...
Behavior that both implementations must agree on belongs in [`tests/shared/`](https://github.com/posit-dev/commons/tree/main/tests/shared) at the repository root, which that directory's README defines as the authority. No fixtures exist there yet; they arrive with the provenance and citation code, each alongside the runners that read it.
