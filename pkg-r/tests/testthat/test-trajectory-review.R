...
test_that("answer markers describe verified and untrusted answers", {
  ...
  expect_match(verified, "commons-icons/trusted-icon.svg", fixed = TRUE)
  ...
  expect_match(untrusted, "commons-icons/warning-icon.svg", fixed = TRUE)
...
test_that("trajectory messages render provenance and strip unsafe markup", {
  ...
  expect_match(html, "commons-answer-dot-verified", fixed = TRUE)
...
test_that("the viewer filters conversations and follows selection", {
  ...
  shiny::testServer(
    ...
    ),
    {
      ...
      expect_match(output$entries$html, "commons-answer-dot")
