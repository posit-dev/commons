...
By default, the subprocess cannot create network sockets. An application author can opt in to network access with `commons(network = "full")`; the filesystem sandbox remains in place, but R code can then contact services reachable from the deployment and transmit data from result handles. Local daemons listening on abstract Unix sockets stay unreachable either way: the seccomp filters screen the calls that would reach them under the default, and Landlock scopes them on Linux 6.12 and later under full network. Only enable network access when that egress is required and acceptable.
